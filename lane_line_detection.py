#
#
# Udacity Self Driving Nano Degree Advanced Lane Line Finding Software 
# By Nicholas Johnson 
# Data 8/12/17 
# Copyright 2017 
#
...
dst = cv2.undistort(img, mtx, dist, None, mtx)
cv2.imwrite('camera_cal/calibration13.jpg',dst)
...
def abs_sobel_thresh(image, orient='x', sobel_kernel=ksize, thresh=(0, 255)):
...
def mag_thresh(image, sobel_kernel=ksize, mag_thresh=(0, 255)):
...
def dir_threshold(image, sobel_kernel=ksize, thresh=(0, np.pi/2)):
...
gradx = abs_sobel_thresh(image, orient='x', sobel_kernel=ksize, thresh=(128, 255))
grady = abs_sobel_thresh(image, orient='y', sobel_kernel=ksize, thresh=(225, 255))
...
# and applies a threshold.
def dir_thresh(img, sobel_kernel=7, thresh=(0, 0.08)):    
    # Apply the following steps to img
    # 1) Convert to grayscale
    gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    # 2) Take the gradient in x and y separately
...
####################### RADIUS OF CARVATURE AND DISTANCE CALCULATIONS ###################
